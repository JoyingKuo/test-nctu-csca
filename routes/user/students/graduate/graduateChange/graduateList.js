...
var StudentId = getStudentId.getStudentId.studentId_post;
...
router.post('/students/graduate/graduateChange/graduateList', StudentId, StudentProfile, queryFree, queryGeneral, queryPass, queryChange, queryCourse, queryNow,queryRule, processOther, processCS, currentOther, currentCS,  processRestore, processResult, function(req, res) {
    var courseResult = res.locals.courseResult;
    var studentId = res.locals.studentId;
    m.ShowUserInfo(studentId, function(err, result) {
        if (err) {
            throw err;
            res.redirect('/');
        } else if (!result) {
            ...
            //res.send(list);
            m.SetStudentGraduate(list, function(err, result) {
                if (err) {
                    throw err;
                    res.redirect('/');
                } else if (!result) {
